<script lang="ts">
	...
	const looks = {
		...
		ai: { emoji: '😢', title: 'AI wins', panel: 'bg-pop/15 ring-pop', text: 'text-pop-dark' },
...
{#if result}
	...
	>
		...
		<p class="text-ink text-xl font-bold">
			You {result.playerPoints} — {result.aiPoints} AI
		</p>
		<span class="text-ink/60 text-sm font-bold capitalize">{result.difficulty} mode</span>
		<p class="text-ink/60 text-sm font-semibold">Hit <strong>New Game</strong> to play again!</p>
	</div>
{/if}
