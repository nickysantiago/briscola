...
test('hard mode: trumps a high-point lead (Ace) when following', () => {
  const s = {
    ...
    gptHand: [{ suit: 'Oros', value: 4 }, { suit: 'Copas', value: 12 }],
  ...
  assert.deepEqual(s.gptHand[idx], { suit: 'Oros', value: 4 }); // spends the low trump
});
...
test('hard mode: discards cheap (does NOT trump) a zero-point lead in the early game', () => {
  const s = {
    trumpSuit: 'Oros',
    playerLeads: true,
    gptHand: [{ suit: 'Oros', value: 1 }, { suit: 'Copas', value: 2 }], // trump Ace + junk
  ...
  assert.deepEqual(s.gptHand[idx], { suit: 'Copas', value: 2 }); // saves the trump Ace
});

test('hard mode: delegates to the endgame solver when deck empty and hand fully known', () => {
  const all = allCards();
  const gptHand = [all[0], all[1]];     // 2 cards
...
test('solveEndgame: a single forced trump win returns the only index', () => {
  const s = { trumpSuit: 'Oros', gptHand: [{ suit: 'Oros', value: 1 }] };
...
test('solveEndgame: when responding, grabs the trick it can win for points', () => {
  // GPT responds to player's Ace-of-Copas lead (11 pts). GPT holds the trump 2
  // (can win the 11) and a junk card. Winning nets +11 and keeps the lead.
  const s = {
    trumpSuit: 'Oros',
    gptHand: [{ suit: 'Bastos', value: 2 }, { suit: 'Oros', value: 2 }]
  };
  ...
  assert.deepEqual(s.gptHand[idx], { suit: 'Oros', value: 2 }); // trump to capture the Ace
});

test('computeUnseen: excludes the AI hand and all played cards', () => {
  const s = {
    gptHand: [{ suit: 'Oros', value: 1 }],
